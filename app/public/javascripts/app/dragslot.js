//调用：dragslot(window.jQuery, window, document);
function dragslot($, window, document, undefined){
	...
	Dragslot.prototype = {
		init : function(){
			...
			slotContainer.element.unbind(eStart).on(eStart, dragStartEvent);
			$(window).unbind(eMove).on(eMove, dragMoveEvent);
			$(window).unbind(eEnd).on(eEnd, dragEndEvent);
